function showList() {
    ...
    if ($('#category_listing').length) {
        var url = '';
        url = SITE_URL + '/' + 'health-category-data';

        var table;
        table = $('#category_listing').DataTable({
            bDestroy: true,
            processing: true,
            deferRender: true,
            serverSide: true,
            ...
            columns: [
                {
                   ...
                   render: function (data, type, row) {
                        return    '<input type="checkbox" value= "'+row.category_id+'" name=id[] />';
                    }
                },
                {
                    data: 'name',
        ...
        // Handle Delete Codes
        $('.btn-deleteCategoy').on('click', function (e) {
            e.preventDefault();
            var ele = $(this);
            var ids = Array();
...
function deleteCategory(ids) {
    ...
    $.ajax({
        type: "POST",
        url: SITE_URL + "/delete-health-category",
        data: {
            ids: ids
        },
        success: function (response) {
            ...
            if (response.success == true) {
                window.location.href = SITE_URL + '/health-category-listing';
            } else {
                if (response.message == '') {
                    $.each(response.result, function (key, value) {
                        $('#category_list .alert-danger').append($("<li/>", {
                            text: value
                        }));

                    });
                } else {
                    $('#category_list .alert-danger').html('<li>' + response.message + '</li>');
                }

                $('#category_list .alert-danger').show();
        ...
        error: function (request, status, error) {
            $(".ui-loader-background").hide();
            $('#category_list .alert-danger').html('<li>Something went wrong. Please contact Benefit Wellness</li>').show();
        }
    });
    return false;
...
function openEditWorkPage(id) {
    window.location.href = SITE_URL + '/health-category?id=' + id;
}

function save_category_form_submit() {
    $(".ui-loader-background").show();
    var formData = new FormData($('#save_category_form')[0]);
    $('#category_list .alert').hide().html('');

    $.ajax({
        type: "POST",
        url: SITE_URL + "/save-health-category",
        data: formData,
        async: true,
        cache: false,
        contentType: false,
...
$(document).ready(function () {
    ...
    if ($("#save_category_form").length > 0) {
        
        $("#save_category_form").trigger('reset');

        var save_category_form = $("#save_category_form");
        save_category_form.validate({
