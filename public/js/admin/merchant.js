function showList() {
    ...
    if ($('#merchant_listing').length) {
        var url = '';
        url = SITE_URL + '/' + 'merchant-data';

        var table;
        table = $('#merchant_listing').DataTable({
            bDestroy: true,
            processing: true,
            deferRender: true,
            serverSide: true,
            ...
            columns: [
                {
                    data: 'master_merchant_id',
                    name: 'master_merchant_id',
                   render: function (data, type, row) {
                        return    '<input type="checkbox" value= "'+row.master_merchant_id+'" name=id[] />';
                    }
                },
                {
                    data: 'merchant_name',
        ...
        // Handle Delete Codes
        $('.btn-deleteMerchant').on('click', function (e) {
            e.preventDefault();
            var ele = $(this);
            var ids = Array();
        ...
        // Handle Edit Event
        $('.btn-editMerchant').on('click', function (e) {
            e.preventDefault();
            var ele = $(this);
            var ids = Array();
            // Iterate over all checkboxes in the table
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success == true) {
                window.location.href = SITE_URL + '/merchant-listing';
            } else {
                if (response.message == '') {
...
function openEditMerchantPage(id) {
    window.location.href = SITE_URL + '/merchant?id=' + id;
}

function save_merchant_form_submit() {
    $(".ui-loader-background").show();
    var formData = new FormData($('#save_merchant_form')[0]);
    $('#work_list .alert').hide().html('');

    $.ajax({
        type: "POST",
        url: SITE_URL + "/save-merchant",
        data: formData,
        async: true,
        cache: false,
        contentType: false,
...
$(document).ready(function () {
    showList();
    // save insurance
    if ($("#save_merchant_form").length > 0) {
        $("#save_merchant_form").trigger('reset');

        var save_merchant_form = $("#save_merchant_form");
        save_merchant_form.validate({
